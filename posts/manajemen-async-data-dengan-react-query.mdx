...
category: React
category_color: blue
title: "Manajemen asynchronous data  dengan React Query"
description: "Disini kita akan belajar bagaimana react-query mempermudah bagaimana kita mengatur asynchronous data di aplikasi react"
...
- Minimal harus membuat 3 state baru lagi (_loading_, _error_, _data_) ketika akan melakukan penarikan data yang berbeda (kasus ini dapat diextract menjadi reusable react hooks, tapi kali ini kita bisa menggunakan react query)
...
- **_Server State_**, semisal data profile, data transaksi dll. maka akan dimanage oleh React Query.
...
ikuti instruksi dari installer tersebut, dan simpan projectmu di folder yang di ingikan, semisal `latihan-react-query`

langkah selanjutnya yaitu menginstall React Query dan Axios untuk API client

```bash
 $ npm i react-query axios
```

Kemudian untuk mengkonfigurasi React Query, ubah file `pages/_app.js` menjadi seperti berikut
...
kemudian hapus isi file `index.js`, dan replace dengan kodingan berikut
...
hook `useQuery` dia akan mengembalikan nilai semisal isLoading, IsError, isFetching, data dll. yang akan berguna di code JSX mu. kamu bisa melihat [didocs React Query](https://react-query.tanstack.com/reference/useQuery)  nilai apa saja dikembalikan hooks tersebut.
...
ketika kita menghapus sebuah user dan sukses, maka kita akan tahu bahwa data yang tampil saat ini tidak bisa dikatakan valid lagi, maka kita perlu meng invalidate nya secara manual, dan React Query akan otomatis menarik data yang baru secara otomatis. perhatikan bahwa function `invalidateQueries` ini menggunakan keys sebagai pembeda setiap data. 
