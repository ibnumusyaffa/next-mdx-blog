...
## Menangani spesifik error dengan Sentinel error
...
## Menambahkan ekstra informasi dengan custom error type
...
func doSomething(level int) error {
    ...
    //kita juga bisa membuat error tanpa function constructor, sebagai contohnya
...
### Mengecek wrapped sentinel error dengan errors.Is
...
### Mengecek wrapped custom error dengan errors.As
...
### Menge-wrap suatu errors dengan custom error type
...
// tambahkan method ini ke custom error type dengan return wrapped error
...
```
di line **41** kita bisa mengecek seperti biasa menggunakan `errors.Is` untuk sentinel error atau `errors.Is` jika yang diwrap adalah custom error type lain 
